...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!interaction.customId.startsWith("bulb")) return;
    ...
    if (interaction.customId === "bulb_off") {
      powerState = "off";
    } else if (interaction.customId === "bulb_on") {
      powerState = "on";
    } else if (interaction.customId === "bulb_info") {
      try {
        const res = await sendUdpDiscovery(udpMessage);
        let bulbStats = res.res.split("+").join("");
        let bulbData = {
          power: bulbStats.match(/power: (\w+)/)[1],
        ...
        const joinedString = Object.entries(bulbData)
    ...
    try {
      // const res = await sendUdpDiscovery(udpMessage);
      const telnetCommand = `{"id": 0, "method": "set_power", "params": ["${powerState}", "smooth", 500]}\r\n`;

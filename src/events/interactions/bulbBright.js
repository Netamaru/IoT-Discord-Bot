...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (
      interaction.customId !== "bright_plus" &&
      interaction.customId !== "bright_min"
    )
      return;
    ...
    let val;
    if (interaction.customId === "bright_plus") {
      val = 10;
    } else if (interaction.customId === "bright_min") {
      val = -10;
    } else {
      return;
    }
    ...
    try {
      ...
      const telnetCommand = `{"id": 0, "method": "set_bright", "params": [${
        bulbData.brightness + val
      }, "smooth", 500]}\r\n`;
      await sendTelnetCommand(response.ip, telnetCommand);
      interaction.deferUpdate();

      data.brightness = bulbData.brightness + val;
